import React from 'react';

export default function Footer() {
  const currentYear = new Date().getFullYear();

  return (
    <footer id="contact" className="bg-krone-black pt-20 pb-8 border-t border-krone-charcoal select-none">
      <div className="max-w-7xl mx-auto px-6 md:px-24">
        
        {/* Top Row: 3 columns */}
        <div className="grid grid-cols-1 md:grid-cols-[40%_35%_25%] gap-12 md:gap-8 pb-16">
          
          {/* Column 1: Brand & Tagline */}
          <div className="flex flex-col items-start">
            <div className="flex items-baseline mb-4">
              <span className="font-display text-[26px] font-black text-krone-amber tracking-tight">
                KRONE
              </span>
              <span className="font-body text-[13px] text-krone-white uppercase tracking-[0.2em] ml-2 font-medium">
                CONSTRUCT
              </span>
            </div>
            <p className="font-body text-sm text-krone-concrete max-w-xs leading-relaxed">
              Built to last a century. Engineering and architectural marvels with absolute precision since 2001.
            </p>
          </div>

          {/* Column 2: Navigation Links (2 columns nested) */}
          <div className="grid grid-cols-2 gap-4">
            {/* Nav Column A */}
            <div className="flex flex-col space-y-3">
              <span className="font-body text-xs font-semibold uppercase tracking-[0.15em] text-krone-concrete/50 mb-1">
                Explore
              </span>
              <a
                href="#work"
                className="font-body text-xs text-krone-concrete hover:text-krone-amber transition-colors duration-300 w-max"
              >
                Our Work
              </a>
              <a
                href="#services"
                className="font-body text-xs text-krone-concrete hover:text-krone-amber transition-colors duration-300 w-max"
              >
                Services
              </a>
              <a
                href="#about"
                className="font-body text-xs text-krone-concrete hover:text-krone-amber transition-colors duration-300 w-max"
              >
                Our Ethos
              </a>
            </div>

            {/* Nav Column B */}
            <div className="flex flex-col space-y-3">
              <span className="font-body text-xs font-semibold uppercase tracking-[0.15em] text-krone-concrete/50 mb-1">
                Legal
              </span>
              <a
                href="#"
                className="font-body text-xs text-krone-concrete hover:text-krone-amber transition-colors duration-300 w-max"
              >
                Privacy Policy
              </a>
              <a
                href="#"
                className="font-body text-xs text-krone-concrete hover:text-krone-amber transition-colors duration-300 w-max"
              >
                Terms of Use
              </a>
              <a
                href="#"
                className="font-body text-xs text-krone-concrete hover:text-krone-amber transition-colors duration-300 w-max"
              >
                Office Location
              </a>
            </div>
          </div>

          {/* Column 3: Contact */}
          <div className="flex flex-col space-y-4">
            <span className="font-body text-xs font-semibold uppercase tracking-[0.15em] text-krone-amber">
              GET IN TOUCH
            </span>
            <div className="flex flex-col space-y-2 font-body text-sm text-krone-white/90">
              <a
                href="mailto:info@kroneconstruct.com"
                className="hover:text-krone-amber transition-colors duration-300"
              >
                info@kroneconstruct.com
              </a>
              <a
                href="tel:+254700000000"
                className="hover:text-krone-amber transition-colors duration-300"
              >
                +254 700 000 000
              </a>
              <p className="text-krone-concrete text-xs leading-relaxed mt-1">
                Krone Plaza, 4th Floor<br />
                Mombasa Road, Nairobi, Kenya
              </p>
            </div>
          </div>

        </div>

        {/* Thin horizontal rule in krone-charcoal */}
        <div className="w-full h-[1px] bg-krone-charcoal mb-8" />

        {/* Bottom Bar */}
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          {/* Copyright */}
          <span className="font-body text-[11px] text-krone-concrete/60 tracking-wider">
            © {currentYear} KRONE CONSTRUCT LTD. ALL RIGHTS RESERVED.
          </span>

          {/* Social Icons (LinkedIn, Instagram, Twitter) */}
          <div className="flex items-center space-x-6 text-krone-concrete">
            {/* LinkedIn */}
            <a
              href="#"
              className="hover:text-krone-amber transition-colors duration-300"
              aria-label="LinkedIn"
            >
              <svg
                className="w-[18px] h-[18px] fill-current"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
              </svg>
            </a>

            {/* Instagram */}
            <a
              href="#"
              className="hover:text-krone-amber transition-colors duration-300"
              aria-label="Instagram"
            >
              <svg
                className="w-[18px] h-[18px] fill-current"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
              </svg>
            </a>

            {/* Twitter / X */}
            <a
              href="#"
              className="hover:text-krone-amber transition-colors duration-300"
              aria-label="Twitter"
            >
              <svg
                className="w-[18px] h-[18px] fill-current"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
              </svg>
            </a>
          </div>
        </div>

      </div>
    </footer>
  );
}
